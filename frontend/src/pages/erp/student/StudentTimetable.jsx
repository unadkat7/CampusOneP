    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Loading weekly timetable...</span>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/80 dark:border-slate-800 pb-5">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">Class Routine</span>
            </div>
        ...
        {timetable.length === 0 ? (
          <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-12 text-center space-y-3">
            ...
            <p className="text-sm font-bold text-slate-800 dark:text-slate-200">No Timetable Scheduled</p>
            <p className="text-xs text-slate-500 dark:text-slate-400">Class schedules for this semester have not been assigned yet.</p>
          </div>
        ) : (
              ...
              return (
                <div key={day} className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-5 shadow-xs space-y-3">
                  <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                    ...
                    <span className="text-xs text-slate-400 dark:text-slate-500 font-semibold">{slots.length} Lectures</span>
                  </div>
                    ...
                    {slots.map((slot, i) => (
                      <div key={i} className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-700/60 space-y-2 hover:border-indigo-400 dark:hover:border-indigo-500 transition-colors">
                        ...
                        <h3 className="text-sm font-bold text-slate-900 dark:text-white leading-snug">
                          {slot.courseId?.name || "Subject Lecture"}
                        </h3>
                        ...
                        <div className="pt-2 border-t border-slate-200/60 dark:border-slate-700/60 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 font-medium">
                          ...
                          {slot.facultyId && (
                            <span className="text-[11px] font-semibold text-slate-600 dark:text-slate-400 truncate max-w-[120px]">
                              {slot.facultyId.firstName} {slot.facultyId.lastName}
                            </span>
                          )}
                        </div>
                      </div>
